...
func TestAppIDAuthMethod(t *testing.T) {
	mount := "app-id"
	token := "sometoken"
	appID := "alice"

	client := fakeVault(t, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "/v1/auth/"+mount+"/login/"+appID, r.URL.Path)
	...
	m = AppIDAuthMethod("some_id", "", "")
	err = m.Login(ctx, client)
	assert.Error(t, err)

	m = AppIDAuthMethod(appID, "s", "")
	err = m.Login(ctx, client)
	assert.NoError(t, err)
	assert.Equal(t, token, client.Token())

	mount = "di-ppa"
	m = AppIDAuthMethod(appID, "s", mount)
	err = m.Login(ctx, client)
	assert.NoError(t, err)
	assert.Equal(t, token, client.Token())
}
